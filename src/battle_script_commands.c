...
const ALIGNED(4) u8 gBattlePalaceNatureToMoveGroupLikelihood[NUM_NATURES][4] =
{
	...
	[NATURE_BRAWNY]  = PALACE_STYLE(61,  7, 61,  7), // 32% support >= 50% HP, 32% support < 50% HP
	...
	[NATURE_WILD]    = PALACE_STYLE(30, 58, 30, 58), // 12%, 12%
    ...
    [NATURE_WISE]    = PALACE_STYLE(56, 22, 56, 22)  // 22%, 22%
};
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (atkAbility == ABILITY_HUSTLE && IS_MOVE_PHYSICAL_BIASED(move, battlerAtk))
    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
    {
        ...
        else
        {
            ...
            else // hp goes down
            {
                ...
                if (IS_MOVE_PHYSICAL_BIASED(gCurrentMove, gBattlerAttacker) && !(gHitMarker & HITMARKER_x100000) && gCurrentMove != MOVE_PAIN_SPLIT)
                {
                ...
                else if (!IS_MOVE_PHYSICAL_BIASED(gCurrentMove, gBattlerAttacker) && !(gHitMarker & HITMARKER_x100000))
                {
...
static bool32 ClearDefogHazards(u8 battlerAtk, bool32 clear)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        u32 *sideStatuses = (u32*)((u32) gSideStatuses[i]);
