...
{
    [NATURE_WILD]   = PALACE_STYLE(61,  7, 61,  7), // 32% support >= 50% HP, 32% support < 50% HP
    ...
    [NATURE_BRAWNY]  = PALACE_STYLE(56, 22, 56, 22), // 22%, 22%
    ...
    [NATURE_NEUTRAL] = PALACE_STYLE(34, 11, 29, 11), // 55%, 60%
    ...
    [NATURE_WISE] = PALACE_STYLE(30, 58, 30, 58), // 12%, 12%
    ...
    [NATURE_GUARDED]  = PALACE_STYLE(56, 22, 56, 22)  // 22%, 22%
};
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (atkAbility == ABILITY_HUSTLE && IS_MOVE_PHYSICAL(move, battlerAtk))
...
static void Cmd_datahpupdate(void)
{
    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
    {
        ...
        {
            //gBattleMons[gActiveBattler].species = gDisguiseForm[gBattleMons[gActiveBattler].species];
        ...
        else
        {
            ...
            else // hp goes down
            {
                ...
                if (IS_MOVE_PHYSICAL(gCurrentMove, gBattlerAttacker) && !(gHitMarker & HITMARKER_x100000) && gCurrentMove != MOVE_PAIN_SPLIT)
                {
                ...
                else if (!IS_MOVE_PHYSICAL(gCurrentMove, gBattlerAttacker) && !(gHitMarker & HITMARKER_x100000))
                {
