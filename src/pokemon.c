...
void CreateBoxMon(struct BoxPokemon *boxMon, u16 species, u8 level, u8 fixedIV, u8 hasFixedPersonality, u32 fixedPersonality, u8 otIdType, u32 fixedOtId)
{
    ...
    else //Player is the OT
    {
        ...
        if (gLastSpeciesEncountered == species)
        {
            f32 val = 1.0 + (f32)gChainEncounter;
            val = ((val * val) + val) / SHINY_ODDS;
            rolls += val;
        }
        ...
        {
            personality = Random32();
            svalue = HIHALF(value) ^ LOHALF(value) ^ HIHALF(personality) ^ LOHALF(personality);
            rolls--;
        } while (svalue >= SHINY_ODDS && rolls > 0);

        if (svalue < SHINY_ODDS)
            gLastSpeciesEncountered = SPECIES_NONE;
    }
...
{
    if (!hoennNum || hoennNum >= SINNOH_DEX_COUNT)	// HOENN_DEX_COUNT
        return 0;
...
// TODO (lilua): fix cry ids being broken
//  return gSpeciesIdToCryId(species - (SPECIES_TREEKO - 1));
    return species;
}
...
u16 SpeciesToPokedexNum(u16 species)
{
    ...
    else
    {
        ...
        if (species <= SINNOH_DEX_COUNT)	// HOENN_DEX_COUNT
            return species;
        return 0xFFFF;
    }
}
...
{
    if (SpeciesToHoennPokedexNum(species) > SINNOH_DEX_COUNT)	// HOENN_DEX_COUNT
        return FALSE;
    return TRUE;
}
